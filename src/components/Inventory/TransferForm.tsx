...
const TransferForm: React.FC = () => {
  ...
  const canView = hasPermission('inventory.transfer.create');
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          ...
          <p className="text-slate-400">You do not have permission to view or create stock transfers.</p>
        </div>
      </div>
    );
  }
  ...
  const fetchAllocations = async (index: number) => {
    ...
    if (!item.item_id || !formData.source_godown_id || item.entered_quantity <= 0) return;

    const res = await fetch(`/api/inventory/fefo-suggestions?itemId=${item.item_id}&godownId=${formData.source_godown_id}&quantity=${item.entered_quantity}`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-8">
        ...
        <h1 className="text-3xl font-bold text-white">New Inter-Godown Transfer</h1>
      </div>

      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-900 p-6 rounded-3xl border border-slate-800">
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Transfer Number</label>
            <input type="text" value={formData.transfer_number} readOnly className="w-full bg-slate-800 border-none rounded-xl px-4 py-3 text-white" />
          </div>
          <div>
        ...
        <div className="bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden">
          <div className="p-6 border-b border-slate-800 flex justify-between items-center">
            <h2 className="text-xl font-bold text-white">Items to Transfer</h2>
