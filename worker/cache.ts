...
export class CacheManager {
  static async get(c: Context, ttl: number) {
    ...
    const cacheVersion = await c.env.KV.get('CACHE_VERSION') || 'v1';
  ...
  static async invalidate(c: Context) {
    ...
    const currentVersion = await c.env.KV.get('CACHE_VERSION') || 'v1';
    const newVersion = `v${parseInt(currentVersion.substring(1)) + 1}`;
    await c.env.KV.put('CACHE_VERSION', newVersion);
